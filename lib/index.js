...
const processSingleBatch = (currentBatch, callBack) => {
  ...
  if (currentDocuments.length > 0) {
    ...
    _.each(currentDocuments, Meteor.bindEnvironment(document => {
      if (document.index) {
 ...
 * */
const connectDB = () => {
  MongoDriver.connect(process.env.SEARCH_MONGO_URL, Meteor.bindEnvironment((error, db) => {
    if (!error) {
 ...
 * */
ESMongoSync.init = (mongoOplogUrl, elasticSearchUrl, callBack, watchers, documentsInBatch) => {
